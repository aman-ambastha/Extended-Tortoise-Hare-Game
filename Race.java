...
public class Race extends JFrame {
	 ...
	 */
	public Race() {
		super("The Tortoise, The Hare, The Panther, & The JellyFish!");
	...
	private void prepareToRace() {
		...
		// Switch Statement, Student Code Part 1
		while (input != 's' && input != 'S') {
			if(input == 'T' || input == 't'){
				...
				racerList.add(new Panther("Panther", START_LINE, yPos));
				yPos = yPos + RACER_SPACE;
			}
			else if(input == 'j' || input == 'J'){
				racerList.add(new JellyFish("JellyFish", START_LINE, yPos));
				yPos = yPos + RACER_SPACE;
			}
			else {
				JOptionPane.showMessageDialog(this, "Please Enter Only Letters Asked (j, t, h, p, s)");
			}
			repaint();
			input = getRacer();
			// get input from user
	...
	private char getRacer() {
		String input = JOptionPane.showInputDialog(this, "Enter a racer:"
				+ "\nt for Tortoise, h for Hare, p for Panther, j for JellyFish " + "\nor s to start the race");
		if (input == null) {
